...
function bind(conn) {
            ...
            for (let { id, name, readOnly } of chats) {
                ...
                if (isGroup) {
                    const metadata = await conn.groupMetadata(id).catch(err => {
                        console.error(`[store.js] Error fetching group metadata for ${id}:`, err.message || err)
                        return null
                    })
                    if (metadata) {
    ...
    conn.ev.on('group-participants.update', async function updateParticipantsToDb({ id, participants, action }) {
        ...
        const groupMetadata = await conn.groupMetadata(id).catch(err => {
            console.error(`[store.js] Error fetching group metadata in participants update for ${id}:`, err.message || err)
            return null
        })
    ...
    conn.ev.on('groups.update', async function groupUpdatePushToDb(groupsUpdates) {
        try {
            for (const update of groupsUpdates) {
                ...
                const metadata = await conn.groupMetadata(id).catch(err => {
                    console.error(`[store.js] Error fetching group metadata in groups.update for ${id}:`, err.message || err)
                    return null
                })
                if (metadata) {
                    chatData.metadata = metadata
        ...
        try {
            ...
            if (isGroup && conn.insertAllGroup) {
                conn.insertAllGroup().catch(err => {
                    console.error('[store.js] Error in insertAllGroup:', err.message || err)
                })
            }
        } catch (e) {
